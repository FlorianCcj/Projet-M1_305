...
file_class.prototype = {
    ...
    date_from_file: function (path, dest_or_from) {
        ...
        if (dest_or_from == "dest") {
            ...
            console.log("Taking date from " + this._dest_d_path + " in destination path : " + this._dest_date);
        }
        if (dest_or_from == "from") {
            this._from_date = stock_date;
            console.log("Taking date from " + this._from_d_path + " in origine path : " + this._from_date);
        }
    },
    // switch the path
    determined_path: function () {
        console.log("Where are checking if the from directory is the more recent or not.");
    ...
    determined_more_recent: function () {
        this.date_from_file(file_test._from_d_path, "from");
        this.date_from_file(file_test._dest_d_path, "dest");
    ...
    compare: function () {
        ...
        comp_by_name_and_meta(this._from_d_path, this._dest_d_path, function (err, create_list, delete_list, update_list) {
            if (err) throw err;
            console.log('Nombre de fichier supprimer : ' + Object.size(delete_list));
            for (var i = 0; i < Object.size(delete_list); i++) {
                console.log(delete_list[i]);
            }

            console.log('\n');
            console.log('Nombre de fichier creer : ' + Object.size(create_list));
            for (var j = 0; j < Object.size(create_list); j++) {
                console.log(create_list[j]);
            }

            console.log('\n');
            console.log('Nombre de fichier update : ' + Object.size(update_list));
...
file_test.run();
//console.log("from date :"+file_test._from_date);
//console.log("from date :"+file_test._from_date[0]);
//console.log("dest date :"+file_test._dest_date);
//console.log("more recent :"+file_test._more_recent);
//file_test.copy_file("copy_test.txt");
//file_test.delete_file("copy_test.txt");
//file_test.create_hidden_file(file_test._from_d_path);
//file_test.compare()